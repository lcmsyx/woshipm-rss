...
def make_rss_item(item):
    ...
    item_xml = '    <item>\n'
    item_xml += '      <title><![CDATA[' + title + ']]></title>\n'
    item_xml += '      <link>https://www.woshipm.com/digest</link>\n'
    item_xml += '      <guid isPermaLink="false">' + guid + '</guid>\n'
    item_xml += '      <description><![CDATA[' + description + ']]></description>\n'
    item_xml += '      <pubDate>' + format_pubdate(created) + '</pubDate>\n'
    item_xml += '    </item>'
    return item_xml
...
def build_rss_header():
    ...
    header += '  <channel>\n'
    header += '    <title>人人都是产品经理快讯</title>\n'
    header += '    <link>https://www.woshipm.com/digest</link>\n'
    header += '    <description>人人都是产品经理每日快讯聚合，每2小时增量更新</description>\n'
    header += '    <language>zh-cn</language>\n'
    header += '    <lastBuildDate>' + now_str + '</lastBuildDate>\n'
    header += '    <atom:link href="https://lcmsyx.github.io/woshipm-rss/rss.xml" rel="self" type="application/rss+xml"/>\n'
    return header
...
def main():
    ...
    save_file(data_file, new_rss, "存档: " + today_str)
